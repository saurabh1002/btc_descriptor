...
def main(input_dir: str = typer.Argument(""), gt_poses_file: str = typer.Argument("")):
    optimizer = PoseGraphOptimizer()
    # Load poses and add them to the graph with the odometry edges
    gt_poses = np.load(gt_poses_file)

    pose_file = os.path.join(input_dir, "poses.npy")
    ...
    ply_path = os.path.join(input_dir, "ply/")
    point_clouds_paths = os.listdir(ply_path)
    local_maps = [
        o3d.io.read_point_cloud(os.path.join(ply_path, x))
        for x in sorted(point_clouds_paths)
        if ".ply" in x
    ]
    closures_file = os.path.join(input_dir, "closures.txt")
    ...
    with o3d.utility.VerbosityContextManager(o3d.utility.VerbosityLevel.Debug) as cm:
        for ids in closures:
            ...
            scan_i = int(ids[2])
            scan_j = int(ids[3])
            source = local_maps[map_i]
            target = local_maps[map_j]
            success = False
            estimate = gicp(source, target, pose, 1.0)
            if estimate.fitness > 0.5:
                success = True
            # success, T = global_registration(source, target, 1.0)
            if success:
        ...
        optimizer.optimize()
        optimizer.write_graph(os.path.join(input_dir, "out.g2o"))
        ...
        np.save(os.path.join(input_dir, "optimized_poses.npy"), poses_opt)
        np.savetxt(os.path.join(input_dir, "gt_poses_kitti.txt"), gt_poses[:, :3].reshape(-1, 12))
        np.savetxt(
            os.path.join(input_dir, "optimized_poses_kitti.txt"), poses_opt[:, :3].reshape(-1, 12)
        )
