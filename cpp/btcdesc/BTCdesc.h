...
public:
    ...
    OctoTree(const ConfigSetting &config_setting) : config_setting_(config_setting) {
        voxel_points_.clear();
        octo_state_ = 0;
        layer_ = 0;
        init_octo_ = false;
...
struct PlaneSolver {
    ...
    bool operator()(const T *q, const T *t, T *residual) const {
        ...
        Eigen::Matrix<T, 3, 1> point_target(T(target_point.x()), T(target_point.y()),
                                            T(target_point.z()));
...
public:
    ...
    BtcDescManager(ConfigSetting &config_setting) : config_setting_(config_setting) {
        current_frame_id_ = 0;
    };
    ...
    void PlaneGeometricIcp(const pcl::PointCloud<pcl::PointXYZINormal>::Ptr &source_cloud,
...
private:
    ...
    void get_project_plane(const std::unordered_map<VOXEL_LOC, OctoTree *> &feat_map,
                           std::vector<Plane *> &project_plane_list);

    void merge_plane(const std::vector<Plane *> &origin_list,
