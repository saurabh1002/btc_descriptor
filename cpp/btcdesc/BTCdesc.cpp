...
void OctoTree::init_plane() {
    ...
    std::for_each(voxel_points_.cbegin(), voxel_points_.cend(), [&](const auto &pi) {
...
void BtcDescManager::PlaneGeometricIcp(
...
void BtcDescManager::get_project_plane(const std::unordered_map<VOXEL_LOC, OctoTree *> &voxel_map,
    ...
    for (auto iter = origin_list.end() - 1; iter != origin_list.begin(); iter--) {
        for (auto iter2 = origin_list.begin(); iter2 != iter; iter2++) {
            ...
            double dis1 = fabs((*iter)->normal_(0) * (*iter2)->center_(0) +
                               (*iter)->normal_(1) * (*iter2)->center_(1) +
                               (*iter)->normal_(2) * (*iter2)->center_(2) + (*iter)->d_);
            double dis2 = fabs((*iter2)->normal_(0) * (*iter)->center_(0) +
    ...
    project_plane_list.reserve(origin_list.size());
    ...
    for (size_t i = 0; i < origin_list.size(); i++) {
        ...
        if (is_merge) {
            ...
            project_plane_list.emplace_back(merge_plane);
        }
    }
    merge_flag.shrink_to_fit();
    project_plane_list.shrink_to_fit();
}

void BtcDescManager::merge_plane(const std::vector<Plane *> &origin_list,
                                 std::vector<Plane *> &merge_plane_list) {
    ...
    std::vector<int> merge_flag;
    merge_flag.reserve(origin_list.size());
    merge_plane_list.reserve(origin_list.size());
    for (size_t i = 0; i < origin_list.size(); i++) {
        auto it = std::find(merge_flag.begin(), merge_flag.end(), origin_list[i]->id_);
        if (it != merge_flag.end()) continue;
        if (origin_list[i]->id_ == 0) {
    ...
    merge_flag.shrink_to_fit();
    merge_plane_list.shrink_to_fit();
}
...
void BtcDescManager::binary_extractor(const std::vector<Plane *> proj_plane_list,
    ...
    temp_binary_list.reserve(proj_plane_list.size() * 100);
    ...
    binary_descriptor_list.reserve(config_setting_.useful_corner_num_);
    if (config_setting_.useful_corner_num_ > temp_binary_list.size()) {
        binary_descriptor_list = temp_binary_list;
    } else {
        std::sort(
            temp_binary_list.begin(), temp_binary_list.end(),
            [&](BinaryDescriptor a, BinaryDescriptor b) { return (a.summary_ > b.summary_); });
...
void BtcDescManager::extract_binary(const Eigen::Vector3d &project_center,
    ...
    std::for_each(input_cloud->begin(), input_cloud->end(), [&](const auto &point) {
        const double dis = fabs(point.x * A + point.y * B + point.z * C + D);
        if (dis >= dis_threshold_min && dis <= dis_threshold_max) {
            ...
            cur_project[0] =
                (-A * (B * pi.y + C * pi.z + D) + pi.x * (B * B + C * C)) / (A * A + B * B + C * C);
            cur_project[1] =
                (-B * (A * pi.x + C * pi.z + D) + pi.y * (A * A + C * C)) / (A * A + B * B + C * C);
            cur_project[2] =
                (-C * (A * pi.x + B * pi.y + D) + pi.z * (A * A + B * B)) / (A * A + B * B + C * C);
    ...
    point_list_3d.points.shrink_to_fit();
    dis_list_2d.shrink_to_fit();
    ...
    for (size_t i = 0; i < point_list_2d.size(); i++) {
        const int x_index = (int)((point_list_2d[i][0] - min_x) / resolution);
        const int y_index = (int)((point_list_2d[i][1] - min_y) / resolution);
    ...
    // calc line or not
    std::vector<Eigen::Vector2i> direction_list = {Eigen::Vector2i(0, 1), Eigen::Vector2i(1, 0),
                                                   Eigen::Vector2i(1, 1), Eigen::Vector2i(1, -1)};
    binary_list.reserve(max_dis_list.size());
    for (size_t i = 0; i < max_dis_list.size(); i++) {
        const Eigen::Vector2i p(max_dis_x_index_list[i], max_dis_y_index_list[i]);
        ...
        if (line_filter_enable) {
            for (int j = 0; j < 4; j++) {
                ...
                const double threshold = dis_array[p[0]][p[1]] - 3;
...
void BtcDescManager::generate_btc(const std::vector<BinaryDescriptor> &binary_list,
    ...
    const int K = config_setting_.descriptor_near_num_;
    std::vector<int> pointIdxNKNSearch(K);
    std::vector<float> pointNKNSquaredDistance(K);
    for (size_t i = 0; i < key_cloud.size(); i++) {
        const pcl::PointXYZ searchPoint = key_cloud.points[i];
        if (kd_tree->nearestKSearch(searchPoint, K, pointIdxNKNSearch, pointNKNSquaredDistance) >
            ...
            for (int m = 1; m < K - 1; m++) {
                for (int n = m + 1; n < K; n++) {
                    ...
                    d_p.y = b * 1000;
                    d_p.z = c * 1000;
                    const VOXEL_LOC position((int64_t)d_p.x, (int64_t)d_p.y, (int64_t)d_p.z);
                    const auto iter = feat_map.find(position);
...
void BtcDescManager::candidate_selector(const std::vector<BTC> &current_BTC_list,
                                        std::vector<BTCMatchList> &candidate_matcher_vec) {
    ...
    std::vector<bool> useful_match(current_BTC_list.size(), false);
    std::vector<std::vector<size_t>> useful_match_index(current_BTC_list.size());
    std::vector<std::vector<BTC_LOC>> useful_match_position(current_BTC_list.size());
    std::vector<size_t> index(current_BTC_list.size());
    std::iota(index.begin(), index.end(), 0);

    std::mutex mylock;
    std::for_each(std::execution::par_unseq, index.cbegin(), index.cend(), [&](const size_t &i) {
        const BTC descriptor = current_BTC_list[i];
        ...
        for (auto voxel_inc : voxel_round) {
            ...
            if ((descriptor.triangle_ - voxel_center).norm() < 1.5) {
                const auto iter = data_base_.find(position);
                if (iter != data_base_.end()) {
        ...
        if (max_vote_index >= 0 && max_vote >= 5) {
            ...
            for (size_t i = 0; i < index_recorder.size(); i++) {
                if (match_list_index[i] == max_vote_index) {
                    ...
                    single_match_pair.first = current_BTC_list[index_recorder[i][0]];
